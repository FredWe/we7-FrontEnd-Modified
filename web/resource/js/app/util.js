define(['bootstrap'], function($){
	...
	module.uploadMultiPictures = function(callback){
		...
		require(['jquery', 'kindeditor'], function($){
			var url = './index.php?c=utility&a=file&do=upload&type=image&ajax=1';
			
			if(options['dest_dir']){
				url += '&dest_dir='+options.dest_dir;
			}
	 ...
	 **/
	module.image = function(val, callback, base64options, options) {
		if(!base64options){
			base64options = '';
		}
		if(!options){
		...
		var queryString = '';
		if(options.dest_dir){
			queryString = '&dest_dir='+options.dest_dir;
		}
		
		var content =
			'<ul class = "nav nav-tabs" style="margin:auto -10px;padding:0 10px;">'+
				'<li class="li_browser"><a href="#image_browser" data-toggle="tab">网络图片</a></li>'+
				'<li class="li_upload"><a href="#image_upload" data-toggle="tab">上传图片</a></li>'+
				'<li class="li_remote"><a href="#image_remote" data-toggle="tab">上传微信</a></li>'+
			'</ul>'+
			'<div class = "tab-content form-horizontal" style="padding:20px 0;">'+
				'<div class="tab-pane" id="image_browser">'+
					'<div class="form-group">' +
				...
				'<div class="tab-pane" id="image_upload">'+
					...
					'<form action="./index.php?c=utility&a=file&do=upload&type=image'+queryString+'&callback=uploaderImageComplete" enctype="multipart/form-data" method="post" target="__image_file_uploader">'+
						'<div class="form-group">' +
					...
					'</form>' +
				'</div>' +
				'<div class="tab-pane" id="image_remote">'+
					'<iframe width="0" height="0" name="__image_file_remote_uploader" style="display:none;"></iframe>' +
					'<form action="./index.php?c=utility&a=file&do=upload&type=image'+queryString+'&callback=uploaderRemoteImageComplete" enctype="multipart/form-data" method="post" target="__image_file_remote_uploader">'+
						'<div class="form-group">' +
							'<label class="col-xs-12 col-sm-2 control-label">上传图片</label>' +
							'<div class="col-sm-10">' +
								'<input type="file" name="file">'+
		...
		if(!options.tabs){
			options.tabs = {'browser': '', 'upload': 'active' }
		}
		
		if(typeof options.tabs.browser != 'undefined'){
			if(options.tabs.browser == 'active' ){
				modalobj.find('.li_browser').addClass('active');
				modalobj.find('#image_browser').addClass('active');
			}
		} else {
			modalobj.find('.li_browser').hide();
			modalobj.find('#image_browser').hide();
		}
		if(typeof options.tabs.upload != 'undefined'){
			if(options.tabs.upload == 'active' ){
				modalobj.find('.li_upload').addClass('active');
				modalobj.find('#image_upload').addClass('active');
			}
		} else {
			modalobj.find('.li_upload').hide();
			modalobj.find('#image_upload').hide();
		}
		if(typeof options.tabs.remote != 'undefined'){
			if(options.tabs.remote == 'active' ){
				modalobj.find('.li_remote').addClass('active');
				modalobj.find('#image_remote').addClass('active');
			}
		} else {
			modalobj.find('.li_remote').hide();
			modalobj.find('#image_remote').hide();
		}
		...
		modalobj.find('button.btn-browser').on('click', function(){
			...
			window.imageBrowser = {
				...
				browser: function(path){
					...
					dialog.find('.modal-body').load('./index.php?c=utility&a=file&do=browser'+queryString+'&options='+base64options+'&type=image&callback=imageBrowser&path=' + path);
				},
				select: function(r) {
				...
				delete: function(file){
					$.get('./index.php?c=utility&a=file&do=delete&type=image'+queryString+'&options='+base64options+'&file=' + file).success(function(dat){
						if(dat == 'success') {
		...
		if(modalobj.find(':hidden[name="options"]').length > 0){
			modalobj.find(':hidden[name="options"]').val(base64options);
		}
		modalobj.find('button.btn-primary').off('click');
		modalobj.find('button.btn-primary').on('click', function(){
			...
			} else if(modalobj.find('.nav.nav-tabs li').eq(1).hasClass('active')) {
				modalobj.find('form')[0].submit();
				module.loading();
			} else {
				modalobj.find('form')[1].submit();
				module.loading();
			}
		});
		
		require(['filestyle'], function($){
			modalobj.find(':file[name="file"]').filestyle({buttonText: '选择图片'});
		});
		...
		window.uploaderRemoteImageComplete = function(r){
			module.loaded();
			if(r && r.media_id && r.type){
				callback({media_id: r.media_id, type: r.type});
				modalobj.modal('hide');
			} else {
				module.message(r.message,'','error');
			}
	...
	module.audio = function(val, callback, base64options, options) {
		...
		var content = 	'<ul class = "nav nav-tabs" style="margin:auto -10px;padding:0 10px;">'+
							'<li class="li_browser"><a href="#audio_browser" data-toggle="tab">网络多媒体</a></li>'+
							'<li class="li_upload"><a href="#audio_upload" data-toggle="tab">上传多媒体</a></li>'+
							'<li class="li_remote"><a href="#audio_remote" data-toggle="tab">上传微信</a></li>'+
						'</ul>'+
						'<div class = "tab-content form-horizontal" style="padding:20px 0;">'+
							'<div class="tab-pane active" id="audio_browser">'+
								'<div class="form-group">' +
									'<label class="col-xs-12 col-sm-2 control-label">媒体地址</label>' +
									'<div class="col-sm-10">' +
										'<div class="input-group">' +
											'<input class="form-control" type="text" id="audio_url" value="' + val + '" placeholder="请输入音乐URL"/>' +
											'<span class="input-group-btn">' +
												'<button class="btn btn-default btn-browser" type="button">浏览多媒体空间</button>' +
											'</span>' +
										'</div>' +
									'</div>' +
								'</div>' +
							...
							'<div class="tab-pane" id="audio_upload">'+
								...
								'</form>' +
							'</div>' +
							'<div class="tab-pane" id="audio_remote">'+
								'<iframe width="0" height="0" name="__audio_file_remote_uploader" style="display:none;"></iframe>' +
								'<form action="./index.php?c=utility&a=file&do=upload&type=audio&callback=uploaderRemoteAudioComplete" enctype="multipart/form-data" method="post" target="__audio_file_remote_uploader">'+
									'<div class="form-group">' +
										'<label class="col-xs-12 col-sm-2 control-label">上传文件</label>' +
										'<div class="col-sm-10">' +
											'<input type="file" name="file">'+
		...
		var footer = 
			'<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>' +
			'<button type="button" class="btn btn-primary">确认</button>';
		var modalobj = module.dialog('请选择多媒体', content, footer, {containerName: 'audio-container'}); 
		
		if(!options.tabs){
			options.tabs = {'browser': 'active', 'upload': '' }
		}
		
		if(typeof options.tabs.browser != 'undefined'){
			if(options.tabs.browser == 'active' ){
				modalobj.find('.li_browser').addClass('active');
				modalobj.find('#audio_browser').addClass('active');
			}
		} else {
			modalobj.find('.li_browser').hide();
			modalobj.find('#audio_browser').hide();
		}
		if(typeof options.tabs.upload != 'undefined'){
			if(options.tabs.upload == 'active' ){
				modalobj.find('.li_upload').addClass('active');
				modalobj.find('#audio_upload').addClass('active');
			}
		} else {
			modalobj.find('.li_upload').hide();
			modalobj.find('#audio_upload').hide();
		}
		if(typeof options.tabs.remote != 'undefined'){
			if(options.tabs.remote == 'active' ){
				modalobj.find('.li_remote').addClass('active');
				modalobj.find('#audio_remote').addClass('active');
			}
		} else {
			modalobj.find('.li_remote').hide();
			modalobj.find('#audio_remote').hide();
		}
		...
		require(['filestyle'], function($){
			modalobj.find(':file[name="file"]').filestyle({buttonText: '选择媒体文件'});
		});
		...
		window.uploaderRemoteAudioComplete = function(r){
			module.loaded();
			if(r && r.media_id && r.type){
				callback({media_id: r.media_id, type: r.type});
				modalobj.modal('hide');
